...
class Approve_Request(Toplevel):
    def __init__(self):
        ...
        #leave id
        self.label_leave_id = Label(self.frame, text="Leave ID", font='arial 16 bold', fg='#e71d36', bg='#ecf39e')
        ...
        #button
        button = Button(self.frame, text="Next", font="arial 12 bold", command=self.request1)
    ...
    def request1(self, event=' '):
        ...
        # button
        button = Button(window, text="OK", font="arial 12 bold", command=self.request2)
        button.place(x=215, y=200)
        ...
        if leave_id != "":
           try:
                ...
                if status == "Pending":
                    ...
                    #for row in con.execute("SELECT medical_leave FROM Leave WHERE id=?", (self.exampleId,)):
                        #self.balance = row[0]
                        #print(self.balance)

                    #for row in con.execute("SELECT casual_leave FROM Employee WHERE id=?", (self.exampleId,)):
                        #balance1 = row[0]
                        #print(balance1)
                        ...
                        #print(3)
                        #con.execute("UPDATE Leave SET casual_leave =? WHERE id= ?", ((self.balance - self.exampleDays), (self.exampleId)))

                messagebox.showinfo("Success", "Regularisation Request approved and updated")
                self.destroy()
